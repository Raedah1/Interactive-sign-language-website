...
def recognise_letter_A(frame, left_hand_landmarks, right_hand_landmarks):
    letter = None

    # calculate the distance between the index finger and thumb of the right hand
    if left_hand_landmarks and right_hand_landmarks:
        thumb_tip = left_hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
        ...
        if distance <= 0.03:
            ...
            for left_tip in finger_up:
                if left_landmarks[left_tip].y < left_landmarks[left_tip - 2].y:
                    finger_stright_status.append(True)
                else:
                    finger_stright_status.append(False)

            for right_tip in finger_down:
                if right_landmarks[right_tip].x > right_landmarks[right_tip - 2].x:
...
def recognise_letter_B(frame, left_hand_landmarks, right_hand_landmarks):
    letter = None
    ...
    if left_hand_landmarks and right_hand_landmarks:
        ...
        # Calculate the distance between the middle fingertip of the left hand and the left hand itself
        ...
        if index_distance <= 0.08 and middle_distance <= 0.08 and ring_distance <= 0.08 and pinky_distance <= 0.10 and left_distance <=0.06 and right_distance <=0.06:
            return "Letter B "

    return None


def recognise_letter_C(frame,right_hand_landmarks):
    letter = None

    if right_hand_landmarks:
        right_thumb_tip = right_hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
        right_index_finger_tip = right_hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP]
        distance = findDistance(right_thumb_tip, right_index_finger_tip)

        right_landmarks = right_hand_landmarks.landmark
        finger_folded_status = []
        for right_tip in finger_down:
            if right_landmarks[right_tip].x < right_landmarks[right_tip - 3].x:
                finger_folded_status.append(True)
            else:
                finger_folded_status.append(False)


        if (0.12<= distance <=0.21):
            if all(finger_folded_status):
                return "Letter C"

    return None

def recognise_letter_D(frame, left_hand_landmarks, right_hand_landmarks):
    letter = None
    ...
    if  right_hand_landmarks and left_hand_landmarks :
        ...
        right_index_finger_tip = right_hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP]
        left_index_tip = left_hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP]
        left_index_mcp = left_hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_MCP]
        distance_1 = findDistance(right_index_finger_tip, left_index_tip)
        distance_2 = findDistance(right_thumb_tip, left_index_mcp)

        right_landmarks = right_hand_landmarks.landmark
        left_landmarks = left_hand_landmarks.landmark
        finger_folded_status = []
        for tip in finger_down:
            if right_landmarks[tip].y > right_landmarks[tip - 3].y:
                if left_landmarks[tip].y < left_landmarks[tip - 3].y:
                    finger_folded_status.append(True)
            else:
                finger_folded_status.append(False)

        if all(finger_folded_status):
            if distance_1 <=0.05 and distance_2 <= 0.09:
                return "Letter D"
    return None


def recognise_letter_P(frame, left_hand_landmarks, right_hand_landmarks):
    letter = None
    ...
    if right_hand_landmarks and left_hand_landmarks:
        ...
        left_index_tip = left_hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP]
        distance_1 = findDistance(right_index_finger_tip, left_index_tip)
        distance_2 = findDistance(right_thumb_tip, left_index_tip)
        distance_3 = findDistance(right_thumb_tip, right_index_finger_tip)
        ...
        if all(finger_folded_status):
            if distance_1 <= 0.05 and distance_2 <= 0.07 and distance_3 <= 0.05:
                return "Letter P"
    return letter
